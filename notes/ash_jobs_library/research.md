...
- Schema additions per workflow resource (generated by AshJobs):
  - `state` column (atom/string) - Current workflow step (defaults to `:state`,
    overridable)
  - `started_at` column (datetime) - When workflow started
  - `completed_at` column (datetime) - When workflow completed
...
**Note:** Simple, clean attribute names with no workflow name prefixes since
there's only one workflow per resource.
...
defmodule AshJobs.Transformers.ResolveConflicts do
  ...
  def transform(dsl_state) do
    ...
    if workflow do
      ...
      # Simple names without workflow prefix
      for {attr_name, expected_type} <- [
