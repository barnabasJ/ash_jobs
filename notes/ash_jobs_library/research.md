...
**Date:** 2025-10-13 **Status:** ARCHIVED - Historical research document

**⚠️ ARCHIVED DOCUMENT:** This document contains the original research phase
exploration and analysis. For the **current simplified architecture** reflecting
all user decisions, see:

- `/notes/ash_jobs_library/summary.md` - **CURRENT ARCHITECTURE** (use this!)
- `/notes/ash_jobs_library/plan.md` - Implementation plan (PARTIALLY OUTDATED)

This document is kept for historical reference and decision rationale only.

## Key Changes Since This Research (See summary.md)

The architecture was **significantly simplified** based on user feedback:

1. **NO ADAPTERS** - Direct integration with ash_state_machine and ash_oban (not
   abstraction layers)
2. **NO ASHJOBS TELEMETRY** - Ash and Oban already provide comprehensive
   telemetry
3. **NO ATTRIBUTE GENERATION** - ash_state_machine generates state attribute,
   Oban tracks timing/errors
4. **NO EXECUTOR ACTIONS** - Users define all actions, AshJobs only adds Change
   module for routing
5. **SIMPLER TRANSFORMERS** - Only 3 transformers + 1 verifier (not 5+ as
   initially proposed)
6. **VERIFICATION + INJECTION** - Missing changes automatically injected with
   warnings (educational approach)
...
- **Simple attribute naming**: Defaults to `:state` for workflow tracking
  (timing and errors tracked by Oban's job table)
...
- `reactor` ~> 0.17.0 - Saga orchestration framework (optional, for patterns)
  ...
  - Note: Ash includes Ash.Reactor built-in, external reactor is for pattern
    reference
  - **⚠️ NOT USED:** AshJobs implements own execution, reactor only for pattern
    inspiration
...
**⚠️ OUTDATED FILE LIST:** This section contains the initial comprehensive file
list (~16+ files). See summary.md for the simplified structure (~7 core files
after user feedback removed adapters and telemetry).
...
- `lib/ash_jobs/workflow_handler.ex` - Behaviour for custom workflow hooks
  ...
  - **⚠️ DEFERRED:** Extension points deferred to v0.2.0+
...
**⚠️ NOTE:** Pattern recommendations in this section reflect initial research.
Final patterns simplified per user feedback (see summary.md).
...
defmodule MyApp.Orders.FulfillmentJob do
  ...
  # Workflow state attribute (GENERATED BY ASH_STATE_MACHINE, not AshJobs)
  # - state: Current step in workflow (defaults to :state, overridable)
  # Note: Workflow timing and errors tracked in Oban's job table
  # **⚠️ CLARIFICATION:** AshJobs configures state_machine DSL, ash_state_machine generates the attribute
...
defmodule AshJobs.Change do
  ...
  def change(changeset, _opts, _context) do
    # 1. Detect which step this action belongs to
    step_info = detect_step(changeset.action.name, changeset.resource)

    if step_info do
      # 2. Determine next step based on on_success routing
      next_step = step_info.on_success
      # Get state attribute from workflow config (defaults to :state)
      state_attr = get_state_attribute(changeset.resource)
  ...
  defp detect_step(action_name, resource) do
    # Use AshJobs.Info to find which step this action belongs to
  ...
  defp get_state_attribute(resource) do
    # Get state attribute from workflow config, defaults to :state
    workflow = AshJobs.Info.workflow!(resource)
    workflow.state_attribute || :state
  end

  defp get_next_step(step_info, routing_type) do
    # Look up the on_success target from step definition
...
- Schema additions per workflow resource (generated by AshJobs):
  - `state` column (atom/string) - Current workflow step (defaults to `:state`,
    overridable)
- Workflow timing and errors tracked in Oban's job table (started_at,
  completed_at, errors)
- User defines their own attributes for data passing between steps (no special
  context management)
- Each job resource needs a relationship to the domain entity (e.g., `order_id`
  foreign key)
...
- `mimic` ~> 1.11 - Mocking library for Elixir
...
### DSL Design (Simplified - One Workflow Per Resource)

```elixir
# In a FulfillmentJob resource - one workflow per resource
workflow do
  # Optional: override state attribute (defaults to :state)
  state_attribute :state
...
attributes do
  attribute :order_id, :uuid            # Relationship to domain entity
  attribute :customer_id, :uuid
...
**Attributes Generated (per workflow resource):**

- `state` (atom) - Current step in workflow: `:load_order`,
  `:validate_inventory`, etc. (defaults to `:state`, overridable via
  `state_attribute`)
  - Also includes terminal states: `:completed`, `:failed`, `:cancelled`

**Note:** Simple, clean attribute naming with no workflow name prefixes since
there's only one workflow per resource. Workflow timing and errors tracked in
Oban's job table (started_at, completed_at, errors).
...
- Example:
  ...
  state_machine do
    initial_states [:load_order]
    state_attribute :state  # Simple, clean attribute naming
    ...
    if step_info do
      ...
      |> Ash.Changeset.after_transaction(fn _changeset, {:ok, record} ->
        ...
        unless next_state in [:completed, :failed, :cancelled] do
          # Trigger names match step names (simple, clean)
          AshOban.run_trigger(record, next_state)
        end
...
defmodule AshJobs.Transformers.ResolveConflicts do
  ...
  def transform(dsl_state) do
    # Get workflow from DSL state (singular - one workflow per resource)
    workflow = Transformer.get_option(dsl_state, [:workflow])
    ...
    if workflow do
      # Get state attribute (defaults to :state)
      ...
      # Note: Workflow timing and errors tracked in Oban's job table
      # (started_at, completed_at, errors) - no additional attributes needed
